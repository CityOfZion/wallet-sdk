...
export class BIP39Encoded {
  ...
  public mnemonic: MnemonicObj = BIP39Encoded.getDefaultMnemonic();
  ...
  bin2hex(b: string): string {
    return b.match(/.{4}/g)!.reduce(function (acc: string, i: string) {
  ...
  encode(mnemonic: string[]): MnemonicObj {
    const encodedMnemonic: MnemonicObj = BIP39Encoded.getDefaultMnemonic();
    encodedMnemonic.phonetic = mnemonic;

    mnemonic.forEach((word) => {
      const idx = this.words.findIndex((x) => x === word);
      const idxBinaryString = idx.toString(2).padStart(12, "0");
      encodedMnemonic.indices.push(idx);
